...
interface GetTemplateCatalogResponse {
	...
	loadBalanceTemplates: Record<
		string,
		Array<{ templateCode: string; trafficSplit: number }>
	>;
}
...
class ApiService {
	static async saveSilentAuthConfig(request: {
		aid: string;
		userName: string;
		password: string;
		refreshToken: string;
	...
	static async getMerchantDetails(
		mid: string
	): Promise<ApiResponse<MerchantDetailsResponse>> {
		return this.fetchApi<MerchantDetailsResponse>(
			API_ENDPOINTS.MERCHANT.GET_DETAILS,
			{
				method: "POST",
	...
	static async updateTemplateCatalog(
	...
	static async getTemplateCatalog(
		request: TemplateCatalogRequest
	): Promise<ApiResponse<GetTemplateCatalogResponse>> {
	...
	static async createPartner(
		template: any
	): Promise<ApiResponse<CreatePartnerResponse>> {
	...
	static async updateInternationalStatus(
		mid: string,
		enabled: boolean
	): Promise<ApiResponse<any>> {
	...
	static async getMerchantConfigInfo(
		aid: string
	): Promise<ApiResponse<MerchantConfig[]>> {
	...
	static async updateMerchantConfig(
		aid: string,
		type: string,
	...
	static async createCommunicationTemplate(
		payload: ICreateTemplate
	): Promise<ApiResponse<any>> {
		return this.fetchApi(API_ENDPOINTS.TEMPLATE.CREATE, {
			method: "POST",
